...
public class QuestionnaireConfiguration : ScriptableObject
{
 public List<Questionnaire> questionnaires = new List<Questionnaire>();
    ...
    public bool ValidateQuestionOptions()
    {
        foreach (var questionnaire in questionnaires)
        {
            foreach (var question in questionnaire.questions)
            {
                if (!IsValidOptionCount(question, questionnaire.type))
                {
                    Debug.LogError($"Question '{question.questionTitle}' in '{questionnaire.QuestionnaireTitle}' exceeds the maximum number of allowed options for its type.");
                    return false;
                }
            }
        }
    ...
    private bool IsValidOptionCount(Question question, QuestionnaireType type)
    {
        int maxOptions = type == QuestionnaireType.Likert ? 12 : 8;
        return question.options.Count <= maxOptions;
    }
...
{
    // if (ValidateLikertScaleOptions() && ValidateQuestionOptions())
    // {
    //     Debug.Log("All questionnaires are valid.");
    // }

        if (ValidateQuestionOptions())
    {
        Debug.Log("All questionnaires are valid.");
    }
}
